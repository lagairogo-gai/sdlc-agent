...
app = FastAPI(title="COMPLETE AI Monitoring System v6",
              description="Ultra-detailed incident management with business and IT operations scenarios",
              version="6.0")
...
workflow_engine = UltraDetailedWorkflowEngine()
...
async def trigger_incident():
    ...
    incident = await workflow_engine.trigger_incident_workflow({})
    return {"incident_id": incident.id, "message": "Incident workflow triggered"}
...
async def get_active_incidents():
    ...
    return {
        "active_incidents": [
            ...
            for incident in workflow_engine.active_incidents.values()
        ]
    }

@app.get("/incident-details/{incident_id}")
    ...
    return {
        ...
        "executions": {
            agent_id: {
                ...
                "logs_available": len(exec.logs) > 0,
                "a2a_messages_sent": exec.a2a_messages_sent,
                "a2a_messages_received": exec.a2a_messages_received,
                "collaboration_sessions": exec.collaboration_sessions
            }
            for agent_id, exec in incident.executions.items()
        }
    }

@app.get("/agent-logs/{incident_id}/{agent_id}")
async def get_agent_logs(incident_id: str, agent_id: str):
    """Get detailed logs for a specific agent's execution"""
    incident = workflow_engine.active_incidents.get(incident_id)
    if not incident:
        return {"error": "Incident not found"}, 404
    
    execution = incident.executions.get(agent_id)
    if not execution:
        return {"error": "Agent execution not found"}, 404
...
async def get_a2a_messages():
    ...
    return {
        ...
        "message_history": [
            ...
            for msg in workflow_engine.a2a_protocol.message_history[-50:]  # Return last 50 messages
        ]
    }
        ...
        "active_contexts": [
            {
                ...
                "agents_contributed": len(ctx.agent_insights)
            }
            for ctx in workflow_engine.mcp_registry.contexts.values()
        ]
    }
